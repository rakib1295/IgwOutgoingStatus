...
namespace IgwOutgoingStatus.Controllers
{
    public class HomeController : Controller
    {
        ...
        //public IActionResult Index(DateTime? specificdate)
        //{
        //    string billingcycle = "";
        //    string partition_day = "";
        //    if(specificdate == null)
        //    {
        //        billingcycle = DateTime.Now.ToString("yyyy") + DateTime.Now.ToString("MM");
        //        partition_day = DateTime.Now.ToString("dd");
        //    }
        //    else
        //    {
        //        billingcycle = specificdate?.ToString("yyyy") + specificdate?.ToString("MM");
        //        partition_day = specificdate?.ToString("dd");
        //    }
        //    IEnumerable<Igw_Loss_Record> lossRecords = _db.Igw_D_Stat_OG_Loss_Record.Where(t => t.Partition_Day == partition_day
        //                && t.BillingCycle == billingcycle).GroupBy(
        //                l => new
        //                {
        //                    l.Dest_Code,
        //                    //l.Dest_Prefix,
        ...
        //                }).ToList().OrderBy(d => d.Total_Loss_BDT);
        //    //.OrderByDescending(e => e.Total_Min);

        //    HomeViewModel homeViewModel = new HomeViewModel();
        //    homeViewModel.Igw_Loss_Record_List = lossRecords;
        //    CultureInfo provider = CultureInfo.InvariantCulture;
        //    homeViewModel.StartDate = DateTime.ParseExact(billingcycle+partition_day,"yyyyMMdd",provider);
        ...
        public IActionResult Index(DateTime? startDate, DateTime? endDate)       
        {
            ...
            int i = DateTime.ParseExact("202206" + "21", "yyyyMMdd", provider).CompareTo(homeViewModel.StartDate);
            i = DateTime.ParseExact("202206" + "19", "yyyyMMdd", provider).CompareTo(homeViewModel.EndDate);
            ...
            Convert.ToInt32(t.BillingCycle + t.Partition_Day) <= Convert.ToInt32(endbillingcycle + endpartition_day)
                        ...
                        }).Select(cs => new Igw_Loss_Record
                        {
                            ...
                            Partition_Day = cs.Key.Partition_Day,
                            //Exchange_Rate = cs.Key.Exchange_Rate
                        }).ToList().Where(p=> p.Total_Loss_BDT < 0).OrderBy(d => d.BillingCycle).OrderBy(d => d.Partition_Day);
            //.OrderByDescending(e => e.Total_Min);
            homeViewModel.Igw_Loss_Record_List = lossRecords;
